...
> **워크스pace:** `C:\Users\dytc7\besma-rev` (FastAPI + Vue3 + SQLite)  
...
- 10명 이하 현장: 소장이 전원 평가 / **11명 이상:** 출역일보 **팀장 열** 기준 분산 평가
...
| B2 | **DECISION-089 문서 vs 코드 불일치** | 문서는 팀장 분산 임계값 **20명**, 코드는 **10명** (`TEAM_LEADER_SPLIT_THRESHOLD`) | 문서 갱신 또는 정책 재확인 |
| B3 | **현장명 하드코딩** | 지게차 폼 기본 현장명 `푸르지오스타셀라49현장` — 로그인 사용자 현장과 미연동 | `auth.user.site_id` → Site.name 연동 |
...
| 파일 | 역할 |
|------|------|
| `backend/app/modules/functional_eval/constants.py` | `TEAM_LEADER_SPLIT_THRESHOLD = 10` |
...
- [ ] `TEAM_LEADER_SPLIT_THRESHOLD` 10 vs 문서 20 정책 확정
- [ ] 지게차 폼 `site_name`을 로그인 현장과 연동
