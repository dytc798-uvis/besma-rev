...
|------|------|
| **Date** | 2026-04-13 |
| **Title** | 대시보드는 처리 알림 우선, HQ/SITE 좌측 메뉴는 주요업무 우선 순서 |
| **Context** | 문서성 메뉴가 상단에 모이면 근로자의견·부적합·DB 조회 등 운영 핵심이 묻히고, 대시보드가 단순 수치 나열이면 당일 조치가 드러나지 않는다. |
| **Options** | A. 메뉴·대시보드 구조 유지 / B. 알림·주요업무 가시성만 재배치(라우트·권한·집계 로직 불변) |
| **Decision** | **B** |
| **Reason** | 오너 지시. `risk-db-overview` 집계는 그대로 두고 카드·섹션 문구만 정리하며, HQ 사이드바는 주요업무/부가 메뉴 그룹으로 나누고 API 메뉴 순서는 그룹 내에서만 적용한다. |
| **Impact Scope** | `HQSafeDashboard.vue`, `SiteDashboard.vue`, `HQSafeLayout.vue`, `SiteLayout.vue`, `hqSidebarMenuGroups.ts`, `ledgerDashboardFilter.ts` |
...
|------|------|
| **Date** | 2026-05-29 |
| **Title** | 기능인제 일용직 인사고과(2-1 기능·2-2 안전) 운영·권한·로그인 |
| **Context** | 일용직 명부 기반으로 현장소장이 소속 현장 근로자에 대해 기능·안전 인사고과를 입력한다. C18(SITE002) 기존 SITE 메뉴와 분리된 전용 평가 흐름이 필요하다. |
| **Options** | A. 일용직 전원 `WORKER` 계정 + 소장 평가 / B. 소장만 BESMA 로그인, 명부는 import만 / **C. 전용 역할 `SITE_FUNCTIONAL_EVAL` + 소장만 로그인 + BESMA 별도 페이지·C18 메뉴 격리** |
| **Decision** | **C** |
| **Reason** | 일용직 개별 계정 없이 소장 1인이 현장 단위로 평가하며, C18 파일럿 메뉴와 타 현장 평가를 UI·권한으로 분리한다. |
| **Impact Scope** | 신규 모듈(평가 회차·명부 import·기능/안전 점수·마감·HQ export), `SITE_FUNCTIONAL_EVAL` 역할·라우트·레이아웃, HQ 정렬·엑셀 다운로드 |
| **Supersedes** | [OPEN-019]/[OPEN-020]의 일용직 대량 계정 범위 — **기능인제 평가 대상 일용직은 개별 BESMA 계정 불필요**(소장 전용) |

**세부 규칙**

| 항목 | 규칙 |
|------|------|
| 명부 파일 | **Excel 통합 문서 (.xlsx)** — `docs/일용직사원리스트_*.xlsx` |
| 소장 판별 | `소속현장코드`별 **직종 = 1** |
| 소장 2명 이상 | 동일 현장코드에서 **나이 많은 사람**(주민번호 기준) |
| 로그인 | ID = **소속현장코드**, PW = **주민번호 앞 6자리(YYMMDD)** |
| Site 매핑 | `소속현장코드` ↔ BESMA `Site.site_code` **1:1** (명부 기준 추가·삭제 반영) |
| 평가 종류 | **2-1 인사고과표(기능)**, **2-2 인사고과표(안전)** |
| 수정 기한 | **마감일 전까지만** 수정 가능 |
| 평가 시작 안내 | **네이버웍스 게시판**(사내망) — BESMA 자동 연동 없음 |
| 현장 UI | 로그인 시 **진행 중 평가표**만 입력 |
| 메뉴 격리 | 타 현장은 **C18 기존 SITE 메뉴 접근 불가** |
| 마감 후 | 본사 **엑셀 다운로드**; HQ UI에서 기능·안전 **등급별·현장별 SORT** |
| **마감일 설정** | 기본 **6월 15일**; **본사(HQ_SAFE) 로그인 시 변경 가능** |
| **안전 수칙 조치** | 심각한 안전 수칙 위반자에 대해 소장이 **위반 항목 선택 → 제재 자동 산출** (마감 전, 사유 입력 가능) |
| **명부 반영** | 본사 **일용직 xlsx → DIFF → 반영** (참조·소장 계정용, [DECISION-088] 평가 목록은 출역일보) |
| **이력 표시** | **영구 퇴출**(`현장/회사 영구`, `영구 출입 금지`)이 아니면 제재·이전 회차 이력 표시 |
| **마일리지** | 우수 의견 마일리지 **UI·API placeholder** (`PREPARED`, 적립 로직 추후) |

**제재 기준 (양식 3-2·제재표)**

| 구분 | 위반 유형 | 제재 규칙 |
|------|-----------|-----------|
| 작업위반 | 안전벨트 미체결, 음주·폭력, 화기작업 방산 미실시, 안전시설 무단 해체, 작업중지 유발, 무단 작업 | **1회 → 당일 현장 퇴출** |
| 안전사고 | 사고 지연 보고, 미보고 후 병원 단독 방문 | **1회 → 현장 영구 퇴출** |
| 안전 지시 불이행 | TBM·QR활동·보호구·공구·이동수칙·흡연구역·정리정돈 등 | **삼진아웃**: 1 구두경고 → 2 교육2h → 3 부현전기 영구 퇴출 |
| 일반 안전수칙 | 기초 수칙·부적절 언행·작업 태만·보호구·작업장 흡연 | **2회제**: 1 경고 → 2 퇴출 |
| 중대 위반 | 음주·흡연·폭행·도박·절도·통제구역 무단출입·지시 불이행·소란 | **1회 → 즉시 퇴출 및 영구 출입 금지** |
...
|------|------|
| **Date** | 2026-05-29 |
| **Title** | 기능인제 평가 대상 = ERP 출역일보(일 1회) · 명부는 참조 |
| **Context** | 일용직 통합 명부(약 7천 명)는 마스터 참조용이며, 실제 당일 평가·제재 입력 대상은 ERP 출역일보에 나온 인원이어야 한다. 평가 기간 중 본사가 매일 출역일보 xlsx를 업로드해 반영한다. |
| **Options** | A. 명부 전원 평가 대상(현행) / **B. 출역일보 일별 스냅샷 → 당일 출역자만 현장 평가 목록** / C. B + 미출역자 평가 저장 차단 |
| **Decision** | **B** (당일 출역 목록 중심; 기간 내 출역 이력·완료 평가는 유지) |
| **Reason** | 현장은 당일 출역자만 평가하고, 명부 7천 명 전체 노출은 운영 부담·오류를 키운다. 주민번호 해시로 기존 평가·제재를 근로자 레코드에 귀속해 재출역 시 점수가 유지된다. |
| **Impact Scope** | `출역일보_*.xlsx` 파서·일별 import API·`functional_eval_attendance_*` 테이블, 현장/HQ 목록·진행률 집계, HQ 출역 업로드 UI |
| **Supersedes** | [DECISION-087] 명부의 “평가 대상 전원” 운영 해석 — 명부는 **참조·소장 계정·site_code 매핑**용 |

**세부 규칙**

| 항목 | 규칙 |
|------|------|
| 출역 파일 | ERP **출역일보 xlsx** (`docs/출역일보_*.xlsx` 형식) — 현장명 블록·성명·주민등록번호 |
| 업로드 주기 | 평가 기간 중 **1일 1회** (본사 HQ). 동일 출역일 재업로드 시 해당 일 스냅샷 **전체 교체** |
| 현장 평가 목록 | **최신 반영된 출역일** 기준, 소속 `site_code` 출역자만 표시 |
| site_code | 출역 행의 RRN → **참조 명부**에 등록된 `소속현장코드` 우선 (명부 미등록 출역자는 import 시 스킵·집계) |
| 명부(7천) | **참조 전용** — 소장·현장코드·인적사항 갱신; 명부 제외만으로 기존 **평가 점수·제재 이력 삭제 안 함** |
| 점수 유지 | 기간 내 한 번이라도 출역·평가된 인원은 `rrn_hash`·`worker_id`에 평가 저장; 이후 미출역일에는 목록에서만 숨김, **재출역 시 기존 등급 유지** |
| HQ 진행률 | 현장별 **기간 내 출역(unique RRN)** 대비 기능+안전 **전체 완료** 인원 (`완료/출역대상`) |
| 마일리지(추후) | 제도 활성화 시 **퇴사 확정** 시 마일리지 초기화 — **1~2일 미출역은 퇴사 아님** (별도 퇴사 판정 후 구현) |
...
|------|------|
| **Date** | 2026-06-02 |
| **Title** | 기능인제 20명 초과 현장은 팀장 분산평가(B) 적용 |
| **Context** | 소장 1인 평가만으로는 대규모 현장 운영 부담이 커져, 근로자 수가 20명을 초과하면 팀장이 팀원을 분담 평가할 수 있어야 한다. 업로드 파일(TXT/XLSX)로 팀장-팀원 매핑을 반영하고 팀장 계정을 자동 발급하는 요구가 확정되었다. |
| **Options** | A. 소장 1인 평가 유지 / **B. 20명 초과 현장만 팀장 분산평가(예외 규칙)** / C. 전 현장 팀장 분산평가 전환 |
| **Decision** | **B** |
| **Reason** | 운영 병목을 해소하면서 기존 소장 중심 흐름을 최대한 유지하기 위해 임계값(20명 초과) 현장에만 분산평가를 적용한다. |
| **Impact Scope** | `functional_eval` 명부/평가 권한(팀장 계정·팀원 배정), HQ 업로드 API(TXT/XLSX), 계정 발급 규칙(`현장코드-2..n`, 초기 비밀번호 주민번호 앞자리), HQ 매핑표 UI |
| **Credential Rule** | 팀장 로그인 ID = `현장코드-순번(2..n)` / 초기 비밀번호 = 주민번호 앞자리(6자리) / 첫 로그인 후 비밀번호 변경 권장 |
...
| 2026-05-29 | Decision 087 추가 — 기능인제 일용직 인사고과(옵션 C·소장 전용·별도 페이지) |
| 2026-05-29 | Decision 087 보강 — 마감일 기본 6/15·HQ 수정, 경고/즉시퇴출 조치 |
| 2026-05-29 | Decision 087 보강 — 제재표(3-2) 기준 위반항목·누적 제재 규칙 확정 |
| 2026-05-29 | Decision 088 추가 — 출역일보 일 1회·명부 참조·평가 점수 유지 |
| 2026-06-02 | Decision 089 추가 — 20명 초과 현장 팀장 분산평가(B)·팀장 계정 자동발급 |
