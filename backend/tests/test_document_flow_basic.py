...
def test_document_upload_review_basic_flow(tmp_path: Path):
    ...
    own_file = client.get(f"/documents/{first_document_id}/file")
    assert own_file.status_code == 200
    assert own_file.content == b"daily upload content"

    current_user["value"] = SimpleNamespace(
        id=2, role=Role.HQ_SAFE, site_id=site_id, login_id="hq_doc_reviewer"
    )
    assert client.get(f"/documents/{first_document_id}/file?disposition=inline").status_code == 200

    current_user["value"] = SimpleNamespace(
        id=3, role=Role.HQ_OTHER, site_id=site_id, login_id="hq_other_viewer"
    )
    assert client.get(f"/documents/{first_document_id}/file").status_code == 403

    current_user["value"] = SimpleNamespace(
        id=4, role=Role.SITE, site_id=site_id + 1, login_id="other_site"
    )
    assert client.get(f"/documents/{first_document_id}/file").status_code == 403

    current_user["value"] = SimpleNamespace(
        id=5, role=Role.WORKER, site_id=site_id, login_id="worker"
    )
    assert client.get(f"/documents/{first_document_id}/file").status_code == 403
    ...
    assert client.get("/documents/99999999/file").status_code == 404

    current_user["value"] = SimpleNamespace(
        id=1, role=Role.SITE, site_id=site_id, login_id="site_doc_manager"
    )
    ...
    # 본사-현장 소통은 본사·현장이 남긴 전체 코멘트와 승인 의견을 함께 제공한다.
    ...
    assert {i["comment_text"] for i in doc_comments} == {
        "현장 확인 요청 사항 메모",
        "본사 검토 예정, 추가 자료는 승인 흐름과 별개로 남깁니다.",
    }
    assert len(doc_approvals) == 1
    assert "승인" in doc_approvals[0]["comment_text"]
    assert all(i["document_type"] == "DAILY_DOC" for i in doc_comm_items)
    assert comm_items[0]["is_read"] is False
    assert all(i["is_read"] is True for i in comm_items[1:])
