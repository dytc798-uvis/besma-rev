...
def build() -> Path:
    ...
    _slide_screen(
        ...
        [
            "평가표 점수: 문항 등급(TOP/MID/LOW/BOTTOM) 합산 → S/A/B/C 등급",
            "첫 제재: 위반 대응 안전항목을 「문제」로 반영(평가표 수정) · penalty_points=0",
            "동일 위반 재발: penalty_points=5 누적 · 평가표는 이미 문제면 유지",
            "포상 승인: bonus_points 합산(+5 기본) · 안전 등급에 가점 반영",
            "표시: effective_total = base_total - penalty + bonus → 등급 재산출",
        ],
        ["C등급/문제 항목: 제재 등록 권장(유도)하나, 미등록 시에도 평가완료 서명은 가능"],
    )
