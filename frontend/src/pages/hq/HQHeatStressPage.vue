  ...
  <div class="hq-heat">
    ...
    <section class="panel">
      <div class="filters">
        ...
        <button v-if="canExportLedger" class="ledger" type="button" :disabled="ledgerDownloading" title="전체 기간 기록을 현장별·날짜별 관리대장으로 출력" @click="downloadLedger">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            <template v-for="group in groupedRecords" :key="group.site_id">
              <tr class="site-group"><td colspan="8">{{ group.site_name }} <small>{{ group.rows.length }}건</small></td></tr>
...
interface Row{id:number;site_id:number;site_name:string;measured_at:string;work_location:string;air_temperature_c:number;relative_humidity_pct:number;apparent_temperature_c:number;risk_level:string;risk_label:string;actual_action_labels:string[];action_compliance:string;status:string}
...
const groupedRecords=computed(()=>{const groups=new Map<number,{site_id:number;site_name:string;rows:Row[]}>();for(const row of records.value){const group=groups.get(row.site_id)||{site_id:row.site_id,site_name:row.site_name,rows:[]};group.rows.push(row);groups.set(row.site_id,group)}return [...groups.values()].sort((a,b)=>a.site_name.localeCompare(b.site_name,"ko"))});
