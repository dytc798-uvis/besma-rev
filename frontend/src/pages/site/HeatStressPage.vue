<template>
  <div class="heat-page">
    ...
    <div class="manual-calc-bar">
      <button type="button" @click="openRecordModal">추가 측정 기록</button>
      <small><strong>하루 기록 횟수 제한 없음</strong> · 필요할 때마다 다시 측정할 수 있으며, 기상 조회가 안 되면 온도·습도를 직접 입력해 계산할 수 있습니다.</small>
    </div>
    ...
    <section class="panel">
      <div class="section-head"><h2>최근 기록</h2><div class="section-actions"><span>{{ records.length }}건</span><button v-if="records.length" class="ledger" type="button" :disabled="ledgerDownloading || auth.isRolePreviewActive" @click="downloadLedger">{{ ledgerDownloading ? "생성 중" : "체감온도관리대장" }}</button></div></div>
...
const auth=useAuthStore(); const records=ref<HeatRecord[]>([]); const saving=ref(false); const confirming=ref(false); const ledgerDownloading=ref(false); const error=ref(""); const showRecorderSignature=ref(false); const confirmingId=ref<number|null>(null);
...
async function downloadLedger(){ledgerDownloading.value=true;error.value="";try{const res=await api.get("/heat-stress/ledger.pdf",{responseType:"blob"});const siteName=(records.value[0]?.site_name||"현장").replace(/[\\/:*?"<>|]+/g,"_");downloadBlobAsFile(res.data,`체감온도관리대장_${siteName}.PDF`)}catch(e:any){error.value=e?.response?.data?.detail||"체감온도관리대장을 생성하지 못했습니다."}finally{ledgerDownloading.value=false}}
...
.manual-calc-bar{display:flex;align-items:center;gap:12px;padding:14px 16px;border:1px solid #99f6e4;border-radius:14px;background:#f0fdfa}.manual-calc-bar small{color:#475569;line-height:1.45}.manual-calc-bar small strong{color:#0f766e}
...
@media(max-width:768px){.manual-calc-bar{align-items:stretch;flex-direction:column}.manual-calc-bar button{width:100%}}
</style>
