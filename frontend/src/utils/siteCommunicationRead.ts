const STORAGE_KEY = "besma_site_communication_read_v1";
...
export function getReadSiteCommunicationKeys(loginId: string | null | undefined): Set<string> {
  if (!loginId) return new Set();
  const arr = parseMap()[loginId] ?? [];
  return new Set(arr.filter((v) => typeof v === "string" && v.trim().length > 0));
}

export function markSiteCommunicationRead(loginId: string | null | undefined, itemKey: string): void {
